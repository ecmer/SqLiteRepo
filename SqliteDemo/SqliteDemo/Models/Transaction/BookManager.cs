...
namespace SqliteDemo.Models.Transaction
{
    public class BookManager
    {
        ...
        {
            Book checkBook = BookPersistence.getBook(delBook);
            if (checkBook == null)
            {
                return false;
            }
            return BookPersistence.DeleteBook(delBook);
        }
         ...
         */
        public static bool ChangeBook(Book changeBook)
        {
            Book checkBook = BookPersistence.getBook(changeBook);
            if (checkBook == null)
            {
                return false;
            }

            return BookPersistence.ChangeBook(changeBook);
